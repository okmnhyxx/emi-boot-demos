...
$.fn.addtabs = function (options) {
    ...
    obj.on('click', 'a[role=tab]', function () {
        if ($(this).attr('href')==='#home') {
            $('.accordion').find('.menu-active').removeClass('menu-active');
        } else {
            var menuId = $(this).attr('href').substr(5, 11);
            sessionStorage.setItem('menu', menuId);
            $('#'+menuId).parents('.accordion').find('.menu-active').removeClass('menu-active');
...
window.Addtabs = {
    ...
    add: function (opts) {
        ...
        if (opts.id.length == 11) {
            sessionStorage.setItem('menu', opts.id);
        }

        $('#'+opts.id).parents('.accordion').find('.menu-active').removeClass('menu-active');
        $('#'+opts.id).addClass('menu-active');
        Addtabs.drop();
    },
    pop: function (id, e) {
        ...
        popHtml.css({
            'top': e[0].offsetHeight + 50 + 'px',
            'left': e[0].offsetLeft + 270 + 'px'
        });
    ...
    close: function (id) {
        ...
        if (obj.find("li.active").attr('id') === "tab_" + id) {
            ...
            var menuItemId = $("#" + id).prev().attr('id').substring(4);
            menuItemId = menuItemId.substr(0, 11);
            if (menuItemId == '') {
                $('.accordion').find('.menu-active').removeClass('menu-active');
            } else {
                $('#'+menuItemId).parents('.accordion').find('.menu-active').removeClass('menu-active');
                $('#'+menuItemId).addClass('menu-active');
            }
        }
        //关闭TAB
        $("#tab_" + id).remove();
