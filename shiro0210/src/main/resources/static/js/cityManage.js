...
requirejs(['common'], function () {
    requirejs(['jquery', 'bootstrap', 'template', 'base', 'paging'], function ($, bootstrap, template, base, Paging) {
        ...
        function getUserList(pageNum, pageSize) {
			base.showLoading("");
            var city = $('#city').val(),status = $('#status').val();
            if (status == 0) {
                status = '';
            } else if (status == 1) {
                status = true;
                ...
                getResult: function (data) {
                    ...
                    if (data.success) {
                        ...
                        if (data.errorCode === 200004) {
                            $('#cityTableBody').html('<tr><td colspan="15">没有符合条件的查询记录</td></tr>');
