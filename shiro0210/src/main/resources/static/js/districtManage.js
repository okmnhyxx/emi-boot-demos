...
requirejs(['common'], function () {
    requirejs(['jquery', 'bootstrap', 'template', 'base', 'paging'], function ($, bootstrap, template, base, Paging) {
        ...
        function getUserList(pageNum, pageSize) {
			base.showLoading("");
            var district = $('#district').val(),status = $('#status').val();
            if (status == 0) {
                status = '';
            } else if (status == 1) {
                status = true;
            ...
            base.sendReq({
                ...
                getResult: function (data) {
                    ...
                    if (data.success) {
                        ...
                        $('#districtTableBody').html(userHtml);
                        ...
                        if (data.errorCode === 200004) {
                            $('#districtTableBody').html('<tr><td colspan="15">没有符合条件的查询记录</td></tr>');
				...
				base.showLoading("");
                    ...
                    var reqData = { 
					 ...
					 if ($('#userModalTitle').text() == '更改状态') {                 
                        url = 'district/changeDistrictState';
